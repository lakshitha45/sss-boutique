...
function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (res.success) {
        // Fetch fresh user state check from context or verify roles redirects
        const resUser = res as any;
        const targetUser = resUser.user || user;
        const userRole = targetUser?.role || (data.email.toLowerCase().startsWith("admin") ? "admin" : "customer");
  ...
  return (
    <div className="max-w-md w-full bg-secondary border border-accent/20 p-8 sm:p-10 shadow-lg font-poppins space-y-8">
      <div className="text-center space-y-2">
        <span className="text-[10px] text-accent font-bold uppercase tracking-[0.2em]">Welcome Back</span>
        <h1 className="font-serif text-3xl font-light tracking-wide text-foreground">Sign In</h1>
        <div className="w-12 h-[1px] bg-accent mx-auto mt-4" />
      </div>

      <div className="bg-zinc-50 border border-zinc-100 p-3.5 text-[11px] text-zinc-500 leading-normal">
        <span className="font-bold text-accent">Concierge Notice:</span>
        <p className="mt-1">
          Enter your registered administrator or customer email. (If database is offline, default mock mode is used for testing.)
        </p>
      </div>
